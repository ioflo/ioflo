...
class BasicTestCase(unittest.TestCase):
    ...
    def testNonBlockingRequestEcho(self):
        ...
        response = httping.Respondent(beta.rxbs, method=method, url=url)
    ...
    def testNonBlockingRequestStreamFancy(self):
        ...
        #response = httping.HttpResponseNb(msgIn, method=method, url=url)
        response = httping.Respondent(beta.rxbs, method=method, url=url,  wlog=wireLogBeta)
    ...
    def testNonBlockingRequestStreamFirebase(self):
        """
        Test NonBlocking Http client
        """
        console.terse("{0}\n".format(self.testNonBlockingRequestStreamFirebase.__doc__))

        from firebase_token_generator import create_token

        auth_payload = {"uid": "backend", "isBackendProcess": True}
        options = {"admin": True}
        # options = {"admin": True}
        # token = create_token("<YOUR_FIREBASE_SECRET>", auth_payload, options)

        token = create_token("2vBL8Bryihn80dvuvMRGCB8u7khdAJwI14TExPNt", auth_payload, options)

        #firebase = Firebase('https://wtdevioscrg.firebaseio.com', auth_token = token)

        #fbUsers = firebase.child("/comments")

        # result = fbUsers.get_params({'orderBy' : 'fullName'})
        #result = fbUsers.get({'orderBy' : '"$key"'})
        # result = fbUsers.get({'orderBy' : '"$key"', 'limitToLast' : 2})
        ...
        hostname = "wtdevioscrg.firebaseio.com"
        ...
        beta = nonblocking.OutgoerTls(ha=eha,
                                      bufsize=131072,
                                      certedhost=hostname,
                                      wlog=wireLogBeta)
        self.assertIs(beta.reopen(), True)
        ...
        port = eha[1]
        method = u'GET'
        url = u'/comments.json?auth={0}'.format(token)

        console.terse("{0} from  {1}:{2} {3} ...\n".format(method, host, port, url))
        headers = odict([('Accept', 'text/event-stream')])
        #headers = odict([('Accept', 'application/json')])
        request =  httping.Requester(host=hostname,
                                     port=port,
                                     method=method,
                                     url=url,
                                     headers=headers)
        msgOut = request.build()
        console.terse("{0}".format(msgOut))
        #lines = [
                   #b'GET /stream HTTP/1.1',
                   #b'Host: 127.0.0.1:8080',
                   #b'Accept-Encoding: identity',
